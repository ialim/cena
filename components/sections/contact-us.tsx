    ...
    setMessageStatus(
      result.ok
        ? {
            type: "success",
            message:
              "Message successfully sent. One of our agents will contact you shortly, thanks.",
          }
        : {
            type: "error",
            message: result.error || "Message not sent, please try again later.",
          }
    );
    setTimeout(() => {
        ...
        <div
          className={`${
            messageStatus.type === "success"
              ? "bg-brand-secondary"
              : "bg-red-600"
          } font-BasierCircle text-white font-light text-sm rounded-lg p-3 my-3 mr-10 absolute top-11 animate-fadeInOut`}
        >
